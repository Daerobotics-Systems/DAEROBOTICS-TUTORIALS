/*
 * Title: DECREASE LED BLINKING RATE
  --------------------------
  ...
  The code in this sketch decreases the blinking rate of an LED connected to 
  Pin 12 with a Tact push button connected to Pin 8 on the Arduino uno board.
...
void setup() {
  ...
  blink_rate = MAX_BLINK_RATE;
  wait = MIN_BLINK_RATE + 100;
...
void loop() {
  ...
  if(button_press())
  {
    ...
    {
     wait = ((blink_rate -= 200)==MIN_BLINK_RATE)?MIN_BLINK_RATE + 100: wait + 200;
     if(wait == MIN_BLINK_RATE + 100)
     blink_rate = MAX_BLINK_RATE;
     start_time = millis();
    }
  }
