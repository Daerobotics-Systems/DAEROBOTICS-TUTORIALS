/*
 * Title: INCREASE LED BRIGHTNESS WITH BUTTON
  --------------------------
 ...
 * Description: 
 ---------------
  In this project the brigtness of an LED is increased using the tact push
  button. That is, when the button is pressed and released multiple times, 
  the LED glows brighter and brighter till it reaches the maximum brigthness and then goes off.
...
void loop() {
  ...
  if(button_press())
  {
    analogWrite(LED_PIN, ((brightness += 20)>MAX_BRIGHTNESS)? brightness = MIN_BRIGHTNESS:brightness);
